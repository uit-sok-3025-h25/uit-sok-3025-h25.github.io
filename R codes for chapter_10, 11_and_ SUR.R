...
# The following R codes cover estimation of linear Statistical models.
# Reading materials on linear Statistical models are available in 
# the course syllabus (see what is called Appendix in chapter 1).  
...
#----------------------------------------------------------
?qchisq
#plot Chi_Square distribution with 1 degrees of freedom
curve(dchisq(x, df=5), from=0, to=20) 
#The x-axis shows the values of a Chi-Square test statistic and 
#the y-axis shows the corresponding value of the probability 
#density function
#-----------------------------------------------------------------------
...
fitsur_joint.hyp <- systemfit(eqSystem, "SUR", data = table.11.3, restrict.matrix = joint.hyp )
summary(fitsur_joint.hyp)

# Are the estimated coeff. the same? Check 
coef(fitsur.r)-coef(fitsur_joint.hyp) # 

#' Which model (the restricted or unrestricted) best fit the data?
...
# or 
lrtest(fitsur_joint.hyp, fitsur) # Keep H0
